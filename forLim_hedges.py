    ...
    def run(self):
        ...
        if ok_result:
            ...
            if c:
                ...
                self.dlg.progressBar.setMaximum(nfiles+4)
                ...
                simplify_PB_path = os.path.join(args["Path_output_merge"], "simplify_PB.shp")
                if not args["Remove_hedges"]:
                    #Create new files
                    singleParts_PB_path = os.path.join(args["Path_output_merge"], "simplify_m2s_PB.shp")
                    simplify_path = os.path.join(args["Path_output_merge"], "simplify.shp")
                    
                    
                    #Processing tools
                    runalg("qgis:difference", f[1], f[0], simplify_PB_path)
                    runalg("qgis:multiparttosingleparts", simplify_PB_path, singleParts_PB_path)
                    runalg("qgis:mergevectorlayers", f[0], singleParts_PB_path, simplify_path)
                    
                    
                    #Set last path
                    lastPath = simplify_path #merged PB and DF
                else:
                    lastPath = f[0] #simplify_FD.shp
                ...
                if args["Remove_hedges"] or args["Remove_polylines"] or args["Remove_polygons"]:
                    ...
                    #Create merge directory
                    args["Path_output_intermediate"] = os.path.join(args["Path_output"],"intermediate")
                ...
                if args["Remove_hedges"]:
                    ...
                    new_file = list()
                    file_type = ["FD","PB"]
                    for f in enumerate([lastPath, os.path.join(args["Path_output_merge"], "simplify_PB&FD.shp")]):
                    ...
                    #Create new files
                    simplify_hedges_PB_path = os.path.join(args["Path_output_intermediate"], "simplify_hedges_PB_withoutFD.shp")
                    simplifyHedges_path = os.path.join(args["Path_output_intermediate"], "simplify_hedges.shp")
                    singleParts_PB_path = os.path.join(args["Path_output_merge"], "simplify_hedges_m2s_PB.shp")
                    
                    
                    #Processing tools
                    runalg("qgis:difference", new_file[1], new_file[0], simplify_hedges_PB_path)
                    runalg("qgis:multiparttosingleparts", simplify_hedges_PB_path, singleParts_PB_path)
                    runalg("qgis:mergevectorlayers", new_file[0], singleParts_PB_path, simplifyHedges_path)
                ...
                if args["Remove_polygons"]:
                    ...
                    simplifyPolygons_path = os.path.join(args["Path_output_intermediate"], "simplify_polygons.shp")
                    
                    
                    #Processing tools
                    runalg("qgis:clip", args["Path_polygons"], mask_path, maskPolygons_path)
                    runalg("qgis:difference", lastPath, maskPolygons_path, simplifyPolygons_path)
                    
                    
                    #Set last path
                    lastPath = simplifyPolygons_path
                ...
                if args["Remove_polylines"]:
                    ...
                    simplifyPolylines_path = os.path.join(args["Path_output_intermediate"], "simplify_polylines.shp")
                    ...
                    runalg("qgis:difference", lastPath, bufferPolylines_path, simplifyPolylines_path)
                    
                    
                    #set last path
                    lastPath = simplifyPolylines_path
                ...
                #Copy last path
                file_name = lastPath.split(".shp")[0]
                ext = [".shp",".shx",".prj",".dbf"]
                for ext_i in ext:
                    if os.path.exists(file_name + ext_i):
                        shutil.copy2(file_name + ext_i, forest_path + ext_i)
                
                
                #Update area field
                ds = driver.Open(forest_path + ".shp")
                layer = ds.GetLayer()
